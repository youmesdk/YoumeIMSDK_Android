...
- 注意：sendId需要注意下，是发送被标记为已读消息的用户Id，而并非调用此接口的用户Id。msgId表示接收到最新一条消息的msgId，有可能发送端发过来的消息有多条。确认消息已读，接收端只给发送端发送最新一条消息的msgId为已读。
...
``` 
 java
    ...
    public final static int PTT_RecognizeFailed = 2030,         // 识别失败
    public final static int PTT_ShortConnectionMode = 2031,     // 短连接模式，不支持发送
